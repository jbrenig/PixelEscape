...
class DefaultMovementController : PlayerMovementController {
    ...
    override fun createTutorialWindow(skin: Skin, screen: GameScreen, maxWidth: Int, maxHeight: Int): Table {
        ...
        val lbl2Line1 = Label("Gravity will make you", skin, StyleNames.LABEL_WHITE)
        lbl2Line1.color = Color.RED

        val crashLine = HorizontalGroup()

        val lbl2Line2 = Label("CRASH", skin, StyleNames.LABEL_WHITE)
        lbl2Line2.color = Color.RED
        lbl2Line2.setFontScale(1.2f)

        val lbl2Line3 = Label("otherwise!", skin, StyleNames.LABEL_WHITE)
        lbl2Line3.color = Color.RED

        crashLine.addActor(lbl2Line2)
        crashLine.addActor(HorizontalSpacer(20f, 20f, 20f))
        crashLine.addActor(lbl2Line3)

        table.add(lbl).center()
        table.row()
        table.add(lbl2Line1).center()
        table.row()
