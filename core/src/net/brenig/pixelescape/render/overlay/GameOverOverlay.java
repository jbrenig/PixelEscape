...
public class GameOverOverlay extends Overlay implements InputProcessor {
	...
	public void render(float delta) {
		...
		//Highscore
		screen.game.batch.begin();
		if (screen.game.userData.getHighScore() == screen.world.player.getScore()) {
			screen.game.getFont().setColor(0, 1, 0, 1);
			screen.game.getFont().getData().setScale(1.2F);
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		if (animationProgress > TIME_TO_WAIT && screen.game.convertToScaled(screenY) > screen.getUiSize() + screen.uiPos) {
			screen.resetToEmptyOverlay();
			screen.restart();
			restartMusic();
			return true;
